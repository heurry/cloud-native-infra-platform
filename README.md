...
面向云原生微服务场景的分布式基础设施管理平台，提供配置中心、服务治理、元数据管理、可观测监控、CI/CD 自动化、弹性扩缩容、分层存储与 AI 运维分析能力。
...
- 部署流水线：发布记录、Canary 状态、回滚入口和 SLO/Benchmark 门禁。
- 可观测监控：请求延迟、TTFT、吞吐、主机资源、GPU、cAdvisor 和 Kubernetes 快照。
- 知识库 / RAG：文档导入、检索、索引重建和业务 Demo 问答。
- Benchmark：直连 vLLM、round-robin proxy、AIBrix Gateway 的并发压测。
